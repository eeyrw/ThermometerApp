...
- (void)senddata:(char*)outbuffer withReportId:(CFIndex)reportId {
    if (!deviceRef) {
        return ;
    }
#ifdef TEST
    NSLog(@"USB发送%02x,%02x",outbuffer[0],outbuffer[1]);
#endif
    IOReturn ret = IOHIDDeviceSetReport(deviceRef, kIOHIDReportTypeOutput, reportId, (uint8_t*)outbuffer, sizeof(outbuffer));
    if (ret != kIOReturnSuccess) {
